...
class UNet_2d(nn.Module):
    ...
    def get_features(self, x):
        ...
        _, decoder_features = self.decoder(encoder_features)  # 获取decoder最后一层特征
        return decoder_features  # [B, 256, H, W]

class UNet_2d_LowFreq(nn.Module):
    def __init__(self, in_chns, class_num, wt_levels=1, wt_type='db1'):
        super(UNet_2d_LowFreq, self).__init__()
        self.lowpass = WTLowPassConv2d(in_channels=in_chns, out_channels=in_chns,
                                       wt_levels=wt_levels, wt_type=wt_type)
        self.base_unet = UNet_2d(in_chns, class_num)

    def forward(self, x):
        x = self.lowpass(x)
        return self.base_unet(x)
        
class UNet_2d_HighFreq(nn.Module):
    def __init__(self, in_chns, class_num, wt_levels=1, wt_type='db1'):
        super(UNet_2d_HighFreq, self).__init__()
        self.highpass = WTHighPassConv2d(in_channels=in_chns, out_channels=in_chns,
                                         wt_levels=wt_levels, wt_type=wt_type)
        self.base_unet = UNet_2d(in_chns, class_num)
