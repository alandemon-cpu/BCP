...
from networks.wavenet2 import HighFreqUNet, LowFreqUNet
...
def highnet(in_chns=1, class_num=2, ema=False):
    net = HighFreqUNet(in_chns=in_chns, class_num=class_num).cuda()
    return net

def lownet(in_chns=1, class_num=2, ema=False):
    net = LowFreqUNet(in_chns=in_chns, class_num=class_num).cuda()
    return net