...
# 小波工具函数
def create_2d_wavelet_filter(wave_type='db1', in_channels=1, out_channels=1, dtype=torch.float):
    """创建2D小波滤波器"""
    w = pywt.Wavelet(wave_type)
    ...
    dec_filters = dec_filters.repeat(1, in_channels, 1, 1)  # [4, in_channels, 2, 2]
    
    return dec_filters

class WaveletProcessor(nn.Module):
    """小波处理模块：分离低频和高频"""
    def __init__(self, wt_type='db1', in_channels=1):
        super().__init__()
        self.in_channels = in_channels
        wt_flt = create_2d_wavelet_filter(wt_type, in_channels, in_channels, torch.float)
    ...
    def forward(self, x):
        ...
        if c != self.in_channels:
            raise ValueError(f"Input channels {c} don't match expected {self.in_channels}")
        
        pad = (self.wt_filter.shape[2] // 2 - 1, self.wt_filter.shape[3] // 2 - 1)
        
        # 使用正确的groups参数
        wt = F.conv2d(x, self.wt_filter, stride=2, groups=self.in_channels, padding=pad)
        wt = wt.reshape(b, c, 4, h // 2, w // 2)
        
        # 提取低频分量 LL [B, C, H/2, W/2]
        low_freq = wt[:, :, 0, :, :]
        
        # 提取高频分量 LH+HL+HH [B, 3*C, H/2, W/2]
        high_freq = wt[:, :, 1:, :, :].reshape(b, -1, h // 2, w // 2)
        
        # 上采样回原始分辨率
        low_freq = F.interpolate(low_freq, scale_factor=2, mode='bilinear', align_corners=False)
        high_freq = F.interpolate(high_freq, scale_factor=2, mode='bilinear', align_corners=False)
        
        return low_freq, high_freq

# 低频网络 - 专门处理低频信息
class HighFreqUNet(nn.Module):
    def __init__(self, in_chns=1, class_num=4):
        super(HighFreqUNet, self).__init__()
        ...
        self.wavelet_processor = WaveletProcessor(wt_type='db1', in_channels=in_chns)
        self.encoder = Encoder(params)
        self.decoder = Decoder(params)
        
        # 高频增强模块 - 输入通道改为3*in_chns
        self.high_freq_enhancer = nn.Sequential(
            nn.Conv2d(3 * in_chns, 32, 3, padding=1),
            nn.BatchNorm2d(32),
            nn.ReLU(inplace=True),
            nn.Conv2d(32, in_chns, 3, padding=1),
            nn.Sigmoid()
        )

    def forward(self, x):
        # 提取高频信息
        _, high_freq = self.wavelet_processor(x)
        
        # 高频增强（将3通道高频映射回原始通道数）
        high_weight = self.high_freq_enhancer(high_freq)
        enhanced_high = x * high_weight  # 增强高频，抑制低频
        
        # UNet处理
        feature = self.encoder(enhanced_high)
        output, _ = self.decoder(feature)
        
        return output

class LowFreqUNet(nn.Module):
    def __init__(self, in_chns=1, class_num=4):
        super(LowFreqUNet, self).__init__()
        ...
        self.low_freq_enhancer = nn.Sequential(
            nn.Conv2d(in_chns, 32, 3, padding=1),
            nn.BatchNorm2d(32),
            nn.ReLU(inplace=True),
            nn.Conv2d(32, in_chns, 3, padding=1),
            nn.Sigmoid()
    ...
    def forward(self, x):
        # 提取低频信息
        low_freq, _ = self.wavelet_processor(x)
        
        # 低频增强
        low_weight = self.low_freq_enhancer(low_freq)
        enhanced_low = x * low_weight  # 增强低频，抑制高频
        
        # UNet处理
        feature = self.encoder(enhanced_low)
        output, _ = self.decoder(feature)
