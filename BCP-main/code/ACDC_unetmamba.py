...
parser.add_argument('--exp', type=str, default='unetmamba', help='experiment_name')
...
def self_train(args, pre_snapshot_path, snapshot_path):
    ...
    # 初始化两个不同的网络
    model1 = BCP_net(in_chns=1, class_num=num_classes).cuda()
    ...
    if os.path.exists(pre_trained_model):
        load_net(model2, pre_trained_model)
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            # 两个模型分别前向传播
            out_unl1 = model1(net_input_unl)
            out_l1 = model1(net_input_l)
            
            out_unl2 = model2(net_input_unl)
            out_l2 = model2(net_input_l)
            
            # 使用另一个模型的预测作为伪标签（代替原来的EMA模型）
            with torch.no_grad():
                # 使用model2的预测作为model1的伪标签
                pre_a2 = model2(uimg_a)
                pre_b2 = model2(uimg_b)
                plab_a2 = get_ACDC_masks(pre_a2, nms=1)
                plab_b2 = get_ACDC_masks(pre_b2, nms=1)
                
                # 使用model1的预测作为model2的伪标签
                pre_a1 = model1(uimg_a)
                pre_b1 = model1(uimg_b)
                plab_a1 = get_ACDC_masks(pre_a1, nms=1)
                plab_b1 = get_ACDC_masks(pre_b1, nms=1)
            
            consistency_weight = get_current_consistency_weight(iter_num//150)
            
            # 计算model1的损失（使用model2的预测作为伪标签）
            unl_dice1, unl_ce1 = mix_loss(out_unl1, plab_a2, lab_a, loss_mask, u_weight=args.u_weight, unlab=True)
            l_dice1, l_ce1 = mix_loss(out_l1, lab_b, plab_b2, loss_mask, u_weight=args.u_weight)
            loss_ce1 = unl_ce1 + l_ce1
            loss_dice1 = unl_dice1 + l_dice1
            loss1 = (loss_dice1 + loss_ce1) / 2
            
            # 计算model2的损失（使用model1的预测作为伪标签）
            unl_dice2, unl_ce2 = mix_loss(out_unl2, plab_a1, lab_a, loss_mask, u_weight=args.u_weight, unlab=True)
            l_dice2, l_ce2 = mix_loss(out_l2, lab_b, plab_b1, loss_mask, u_weight=args.u_weight)
            loss_ce2 = unl_ce2 + l_ce2
            loss_dice2 = unl_dice2 + l_dice2
            loss2 = (loss_dice2 + loss_ce2) / 2
            ...
            writer.add_scalar('info/total_loss', loss1 + loss2, iter_num)
            writer.add_scalar('info/model1_loss', loss1, iter_num)
            writer.add_scalar('info/model2_loss', loss2, iter_num)
            writer.add_scalar('info/consistency_weight', consistency_weight, iter_num)     

            logging.info('iteration %d: total_loss: %f, model1_loss: %f, model2_loss: %f' % 
                        (iter_num, loss1 + loss2, loss1, loss2))
            ...
            if iter_num % 20 == 0:
                ...
                # 显示两个模型的预测
                outputs1 = torch.argmax(torch.softmax(out_unl1, dim=1), dim=1, keepdim=True)
                outputs2 = torch.argmax(torch.softmax(out_unl2, dim=1), dim=1, keepdim=True)
                
                writer.add_image('train/Un_Prediction1', outputs1[1, ...] * 50, iter_num)
                writer.add_image('train/Un_Prediction2', outputs2[1, ...] * 50, iter_num)
                ...
                outputs_l1 = torch.argmax(torch.softmax(out_l1, dim=1), dim=1, keepdim=True)
                outputs_l2 = torch.argmax(torch.softmax(out_l2, dim=1), dim=1, keepdim=True)
                
                writer.add_image('train/L_Prediction1', outputs_l1[1, ...] * 50, iter_num)
                writer.add_image('train/L_Prediction2', outputs_l2[1, ...] * 50, iter_num)
            ...
            if iter_num > 0 and iter_num % 200 == 0:
                ...
                for _, sampled_batch in enumerate(valloader):
                    metric_i1 = val_2d.test_single_volume(sampled_batch["image"], sampled_batch["label"], model1, classes=num_classes)
                    metric_i2 = val_2d.test_single_volume(sampled_batch["image"], sampled_batch["label"], model2, classes=num_classes)
                ...
                for class_i in range(num_classes-1):
                    writer.add_scalar('info/val1_{}_dice'.format(class_i+1), metric_list1[class_i, 0], iter_num)
                    writer.add_scalar('info/val1_{}_hd95'.format(class_i+1), metric_list1[class_i, 1], iter_num)
                    writer.add_scalar('info/val2_{}_dice'.format(class_i+1), metric_list2[class_i, 0], iter_num)
                    writer.add_scalar('info/val2_{}_hd95'.format(class_i+1), metric_list2[class_i, 1], iter_num)

                performance1 = np.mean(metric_list1, axis=0)[0]
                performance2 = np.mean(metric_list2, axis=0)[0]
                
                writer.add_scalar('info/val_mean_dice1', performance1, iter_num)
                writer.add_scalar('info/val_mean_dice2', performance2, iter_num)
                ...
                logging.info('iteration %d : model1 mean_dice : %f, model2 mean_dice : %f' % 
                            (iter_num, performance1, performance2))
